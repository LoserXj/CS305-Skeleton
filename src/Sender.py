...
class Sender (object):
    def __init__(self,host,port,chunk_hash,send_addr,time_out,has_chunks):
        self.host = host
        self.port = port
        self.chunk_hash = chunk_hash
        self.send_addr = send_addr
        self.finish_hand_shake = False       # 是否完成握手定理，如果sender接收到type = 2的数据说明完成握手定理
        self.rwnd = 14                       # 接收端接收窗口大小，初始化为14
        self.window_size = 28                # 发送端窗口大小
        self.cwnd = 14                       # 拥塞窗口大小
        self.send_queue = []                 # 缓存发送数据
        self.base = 1                        # 基序号
        self.next_seq_num = 1                # 下一个序号
        self.ssthresh = 10000.0              # 慢启动阈值
