...
def process_download(sock, chunkfile, outputfile):
    ...
    whohas_header = struct.pack("HBBHHII", socket.htons(recv.rwnd), update_rwnd, 0,
...
def sender_get_type_zero(sock,data,from_addr,update_rwnd,rwnd):
    ...
    if chunkhash_str in config.haschunks:
        ...
        if send == None:
            ...
            send = Sender(config.ip, config.port, chunkhash_str, from_addr, config.timeout,config.haschunks)
            send.rwnd = rwnd
            print(
        ...
        ihave_header = struct.pack("HBBHHII", send.rwnd, update_rwnd, 1, socket.htons(HEADER_LEN),
...
def receiver_get_type_one(sock,data,from_addr,update_rwnd):
    ...
    get_header = struct.pack("HBBHHII", socket.htons(recv.rwnd), update_rwnd, 2, socket.htons(HEADER_LEN),
                             socket.htons(HEADER_LEN + len(get_chunk_hash)), socket.htonl(0), socket.htonl(0))
    get_pkt = get_header + get_chunk_hash
    sock.sendto(get_pkt, from_addr)

def sender_get_type_two(sock,from_addr,update_rwnd):
    global config
    global ex_sending_chunkhash
    global send
    global recv
    ...
    send_size = min(send.rwnd,send.cwnd)
    for i in range(send_size):
        ...
        if send.next_seq_num + 1 - send.base > send.cwnd:
        ...
        data = Data(msg,send.next_seq_num,1)
...
def receiver_get_type_three(sock,data,update_rwnd,seq):
    ...
    if update_rwnd == 1:
        print("receiver tells sender to update rwnd")
    ...
    if seq <= recv.ack:
        ...
        # 将数据插入到接收缓存队列中
        recv.receive_queue.insert(seq-1-recv.ack,recvData)
        # 如果接收到的序号位于窗口第一个,意味着有部分数据是有序的,则接收窗口移动
        if seq == recv.ack+1:
            receive_queue_len = len(recv.receive_queue)
            for index in range(receive_queue_len):
                if recv.receive_queue[0].seq!=recv.last_byte_read+1:
                    break
                # 已读数据+1
                recv.last_byte_read+=1
                # 已确认的最小有序ack+1
                recv.ack+=1
                recv.recv_data.append(recv.receive_queue[0].msg)
                del recv.recv_data[0]
...
def sender_get_type_four(sock,update_rwnd,ack):
    global config
    global ex_sending_chunkhash
    global send
    global recv
    ...
    for i in range(len(send.send_queue)):
        ...
        if data.seq == ack:
            ...
            if not data.is_resend and not data.is_cal:
                send.time_out_interval = cal_time_out_interval(estimated_RTT=send.estimated_RTT,alpha=send.alpha,dev_RTT=send.dev_RTT,beta=send.beta,time_send=send.pkg_time[i],time_recv=time_recv)
                data.is_cal = True
...
def re_send(sock,update_rwnd,data_resend):
    ...
    # 超时执行重传,更新cwnd和ssthresh的值同时进入慢启动状态
        ...
        print(f"sender changes congestion state from {send.congestion_state} into {0}")
...
def process_inbound_udp(sock):
    ...
    if type == 0:
        ...
        send.rwnd = rwnd
        # 获得需要get的pkt
        sender_get_type_two(sock=sock, from_addr=from_addr, update_rwnd=update_rwnd)
    elif type == 3:
        ...
        # 首先更新sender的rwnd的值
        send.rwnd = rwnd
        if send.base * MAX_PAYLOAD > CHUCK_DATA_SIZE:
            print(f"finished sending {ex_sending_chunkhash}")
            pass
        else:
            sender_get_type_four(sock=sock,update_rwnd=update_rwnd,ack=ack)
