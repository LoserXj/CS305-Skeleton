    ...
    with open(chunkfile, "r") as cf:
        ...
        while i < len(lines):
            ...
            whohas_header = struct.pack("HBBHHII", socket.htons(recv.rwnd), update_rwnd, 0, socket.htons(HEADER_LEN),
                                        socket.htons(HEADER_LEN + len(download_hash)), socket.htonl(0), socket.ntohl(0))
...
def receiver_get_type_three(sock, data, update_rwnd, seq):
    ...
    if seq <= recv.ack:
        ...
        recv.rwnd = recv.receive_buffer - (recv.last_byte_receive - recv.last_byte_read)
        print(
            f"response : {seq} , receiver ack : {recv.ack} , receive queue length : {len(recv.receive_queue)} , receiver rwnd : {recv.rwnd}")
        ack_header = struct.pack("HBBHHII", socket.htons(recv.rwnd), 0, 4, socket.htons(HEADER_LEN),
...
def get_new_ack(ack, time_recv):
    ...
    for i in range(len(send.send_queue)):
        ...
        if data.seq == ack:
            print(
                f"get ACK : {ack} send base : {send.base} send next seq num : {send.next_seq_num} sender cwnd : {send.cwnd} , sender ssthresh : {send.ssthresh} "
                f"")
            data.state = 2
            # 没有被重传的数据，更新超时时间间隔
            if not data.is_resend and not data.is_cal:
                send.time_out_interval = cal_time_out_interval(estimated_RTT=send.estimated_RTT, alpha=send.alpha,
                                                               dev_RTT=send.dev_RTT, beta=send.beta,
...
def process_inbound_udp(sock):
    ...
    rwnd_hton, update_rwnd_hton, Type_hton, hlen_hton, plen_hton, Seq_hton, Ack_hton = struct.unpack("HBBHHII",
                                                                                                     pkt[:HEADER_LEN])
    rwnd, update_rwnd, type, hlen, plen, seq, ack = socket.ntohs(rwnd_hton), update_rwnd_hton, Type_hton, socket.ntohs(
    ...
    if type == 0:
        ...
        receiver_get_type_one(sock=sock, data=data, from_addr=from_addr,
        ...
        if len(recv.recv_data) == 512:
            for i in range(512):
                ex_received_chunk[recv.receive_chunk_hash] += recv.recv_data[i]

        if len(ex_received_chunk[recv.receive_chunk_hash]) == CHUCK_DATA_SIZE:
            ...
            # success = recv.receive_chunk_hash == received_chunkhash_str
